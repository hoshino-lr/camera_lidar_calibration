...
#ifndef METHOD1_COMMON_H
#define METHOD1_COMMON_H
...
void getData(const string lidar_path, const string photo_path, vector<PnPData> &pData) {
    ...
    while(getline(inFile_lidar, lineStr_lidar) && getline(inFile_photo, lineStr_photo)) {
        if (lineStr_lidar.size() > 8 && lineStr_photo.size() > 8) {  // ignore the index
            PnPData pp;
            string str;
        ...
        else if ((lineStr_lidar.size() < 8 && lineStr_photo.size() > 8) || (lineStr_lidar.size() > 8 && lineStr_photo.size() < 8)) {
